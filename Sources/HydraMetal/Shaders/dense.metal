...
/// y = W·x + bias, with W in Q8: `Int8` levels and one BF16 scale per 32 values.
///
/// Levels and scales are **separate runs**, not interleaved — see `Q8`. That is what lets
/// this loop read levels four at a time through `char4` and touch one scale per block,
/// instead of stepping over a scale in the middle of every other vector load.
///
/// The scale factors out of the block: `sum(level_i · x_i) · scale`, one multiply per 32
/// values rather than per value.
kernel void q8_gemv(
    device const char4  *levels   [[buffer(0)]],
    ...
    uint  laneCount [[threads_per_threadgroup]])
{
    ...
    device const char4  *lv = levels + (row * cols) / 4u;
    device const ushort *sc = scales + row * blocksPerRow;
    device const float4 *xv = reinterpret_cast<device const float4 *>(x);

    float acc = 0.0f;
    for (uint b = lane; b < blocksPerRow; b += laneCount) {
        // A block is 32 values: eight char4 against eight float4.
        float4 partial = 0.0f;
        for (uint k = 0; k < 8u; ++k) {
            const char4 q = lv[b * 8u + k];
            partial += float4(float(q.x), float(q.y), float(q.z), float(q.w)) * xv[b * 8u + k];
        }
        acc += (partial.x + partial.y + partial.z + partial.w) * bf16_to_float(sc[b]);
    }
